...
import {
  GET_FOLLOWERS,
  GET_FOLLOWING,
  GET_CREATED_ARTICLES_COUNT,
  GET_READ_ARTICLES_COUNT,
...
export const getFollowersRequest = () => async (dispatch) => {
  try {
    ...
    const {
      data: { followers },
    } = await axios.get(`${apiUrl}/user/followers`, {
...
export const getFollowingsRequest = () => async (dispatch) => {
  try {
    ...
    const {
      data: { followings },
    } = await axios.get(`${apiUrl}/user/followings`, {
      headers: { Authorization: token },
    });
...
export const getArticlesWrittenRequest = () => async (dispatch) => {
  try {
    ...
    const {
      data: { articleCount },
    } = await axios.get(`${apiUrl}/user/articlescount`, {
      headers: { Authorization: token },
    });
...
export const getArticlesReadRequest = () => async (dispatch) => {
  try {
    const token = localStorage.getItem("token");
    const {
      data: { numberOfArticlesRead },
    } = await axios.get(`${apiUrl}/user/readingstats`, {
      headers: { Authorization: token },
    });
...
export const getAllArticlesByUser = (authorId) => async (dispatch) => {
  try {
    ...
    } = await axios.get(`${apiUrl}/articles?authorId=${authorId}`, {
      headers: { Authorization: token },
    });
...
export const getReactionsOnUserArticles = async (dispatch) => {
  try {
    ...
    const {
      data: { likes, dislikes },
    } = await axios.get(`${apiUrl}/user/reactions`, {
      headers: { Authorization: token },
    });
